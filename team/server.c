...
void *thread_function(void *arg) {
	...
	while (1) {
		...
		fprintf(stderr, "\033[1;32m"); //글자색 녹색 변경
		printf("server > ");
...
int main(int argc, char *argv[]) {
	...
	char buf[MAX_LINE + 1]; //client message
	...
	while (1) {
		...
		if (FD_ISSET(listen_sock, &read_fds)) {
			...
			fprintf(stderr, "\033[32m"); //글자색 녹색 변경
...
void addClient(int s, struct sockaddr_in *new_client_addr) {
	...
	fprintf(stderr, "\033[33m"); //글자색 노란색변경
	printf("new client: %s\n", buf); // ip출력
	// 채팅 클라이언트 목록에 추가
